...
# Asegúrate de que tu código con metricas_portafolio, universo, returns y benchmark esté importado
# from tu_codigo import metricas_portafolio, returns_sectors, returns_regions, benchmark_sectors, benchmark_regions, rf_diaria, periodos_año

st.set_page_config(page_title="Comparativa Portafolios", layout="wide")

st.title("💹 Comparativa de Portafolios: Sectores vs Regiones")

# -------------------------------
# Inputs: Pesos de usuario
# -------------------------------
st.header("Ingrese los pesos de su portafolio")

st.subheader("Sectores")
sectores_tickers = universo['sectors']['tickers']
default_sectores = np.round(universo['sectors']['weights'], 2)
user_weights_sectores = st.text_input(
    f"Pesos para {len(sectores_tickers)} sectores (separados por coma)",
    value=",".join([str(x) for x in default_sectores])
)

st.subheader("Regiones")
regiones_tickers = universo['regions']['tickers']
default_regiones = np.round(universo['regions']['weights'], 2)
user_weights_regiones = st.text_input(
    f"Pesos para {len(regiones_tickers)} regiones (separados por coma)",
    value=",".join([str(x) for x in default_regiones])
)
...
if st.button("Calcular métricas"):
    try:
        # Convertir inputs a arrays
        w_sectores = np.array([float(x.strip()) for x in user_weights_sectores.split(",")])
        w_regiones = np.array([float(x.strip()) for x in user_weights_regiones.split(",")])

        # Validar cantidad de pesos
        if len(w_sectores) != len(sectores_tickers):
            st.error(f"Debes ingresar {len(sectores_tickers)} pesos para sectores.")
        elif len(w_regiones) != len(regiones_tickers):
            st.error(f"Debes ingresar {len(regiones_tickers)} pesos para regiones.")
        else:
            ...
            # Mostrar métricas
            # -------------------------------
            st.subheader("📊 Métricas del Portafolio de Sectores")
            df_sectores = pd.DataFrame(metrics_sectores, index=[0]).T
            df_sectores.columns = ["Valor"]
            st.table(df_sectores)

            st.subheader("📊 Métricas del Portafolio de Regiones")
            df_regiones = pd.DataFrame(metrics_regiones, index=[0]).T
            df_regiones.columns = ["Valor"]
            st.table(df_regiones)

            # -------------------------------
            # Comparativa visual
            # -------------------------------
            st.subheader("📈 Comparativa Retorno vs Volatilidad")
            import matplotlib.pyplot as plt

            fig, ax = plt.subplots(figsize=(8,6))
            ax.scatter(df_sectores.loc["vol_anual"].Valor, df_sectores.loc["ret_anual"].Valor, color='blue', s=100, label='Sectores')
            ax.scatter(df_regiones.loc["vol_anual"].Valor, df_regiones.loc["ret_anual"].Valor, color='green', s=100, label='Regiones')
            ax.set_xlabel("Volatilidad Anualizada")
            ax.set_ylabel("Retorno Anualizado")
            ax.set_title("Comparativa Portafolios")
            ax.legend()
            ax.grid(True)
            st.pyplot(fig)
