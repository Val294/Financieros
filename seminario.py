...
def run_app():
    ...
    fig = px.bar(
        ...
        title=f"Comparación por métrica: {metric}"
    )
    st.plotly_chart(fig)
    ...
    # ======== Evaluación del portafolio elegido por el usuario ========
    ...
    if user_weights:
        try:
            ...
            if m["vol"] < np.mean([metrics[p]["vol"] for p in port_names]):
                ...
                comment.append("⚠️ Alta volatilidad comparada con otros portafolios.")
...
# ============================================================
# =========   BLOQUE GLOBAL PARA STREAMLIT   =================
# ============================================================

# 1. Definir portafolios base a evaluar
portafolios = {
    "Regiones": universo["regions"]["weights"],
    "Sectores": universo["sectors"]["weights"],
}

# 2. Definir returns para cada portafolio
returns_dict = {
    "Regiones": returns_regions,
    "Sectores": returns_sectors
}

# 3. Definir benchmark para cada portafolio
benchmark_dict = {
    "Regiones": benchmark_regions,
    "Sectores": benchmark_sectors
}

# 4. Construir las métricas globales
metrics = {}

for name in portafolios.keys():
    metrics[name] = metricas_portafolio(
        returns_dict[name],
        portafolios[name],
        benchmark_dict[name],
        rf_diaria,
