...
def metricas_portafolio(returns: pd.DataFrame,
    ...
    cagr = (cumulative.iloc[-1])**(periodos_año / len(r)) - 1  # tasa compuesta anual (CAGR)
    calmar = cagr / max_dd
...
st.set_page_config(page_title="Optimización de Portafolio", layout="wide")
...
- **Máximo drawdown:** menor valor indica menos riesgo de pérdida extrema.
- **Sortino ratio:** similar a Sharpe, considerando solo las caídas.

Selecciona el portafolio y criterio que te interesa y la app mostrará recomendaciones.
""")

# -------------------------------
# Selección de portafolio y métrica
# -------------------------------
st.header("1️⃣ Comparativa Portafolios")

universos = ["Regiones", "Sectores"]
universo_sel = st.radio("Elige universo:", universos)

criterios = ["Máximo Sharpe", "Mínima Volatilidad", "Máximo Retorno"]
...
# Calculamos portafolios óptimos
...
# Crear arrays de métricas para graficar
...
# -------------------------------
# Gráfica retorno vs volatilidad
# -------------------------------
...
ax.set_title(f"Portafolios Óptimos - {universo_sel}")
ax.grid(True)
st.pyplot(fig)

# -------------------------------
# Mostrar pesos del mejor portafolio según prioridad
# -------------------------------
st.header("2️⃣ Mejor Portafolio según tu prioridad")
if criterio_sel == "Máximo Sharpe":
    best_metrics = m_sharpe
    best_weights = w_sharpe
elif criterio_sel == "Mínima Volatilidad":
    best_metrics = m_min
    best_weights = w_min
else:
    best_metrics = m_maxr
...
st.subheader("Pesos óptimos por activo")
st.bar_chart(pd.Series(best_weights, index=returns.columns))

# -------------------------------
# 3️⃣ Espacio interactivo: ingresar tus propios pesos
# -------------------------------
st.header("3️⃣ Ingrese su propio portafolio")
user_weights = []
st.markdown("Ingrese pesos para cada activo separados por coma, sumando 1 (si no suman 1 se normalizarán automáticamente):")
...
if st.button("Calcular métricas de su portafolio"):
    try:
        ...
        if len(w_user) != returns.shape[1]:
            ...
            st.subheader("Métricas de tu portafolio")
